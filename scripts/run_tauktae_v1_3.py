...
# ------------------------------------------------------------
# NOTE: You will NOT run this now. It is kept for completeness.
try:
    ...
    print("\n⚠ Tauktae run skipped due to computational limits.")
    print(f"Engine error: {e}")
    print("Script updated successfully — safe to use in batch runner.\n")
    merged_all = None
    by_state = {}

# ------------------------------------------------------------
# 5. Save outputs only if run succeeded
# ------------------------------------------------------------
...
with open(os.path.join(outdir, "run_log.txt"), "w", encoding="utf-8") as f:
    f.write(f"CCART v1.3 script prepared for {cyclone_name} ({sid})\n")
    ...
    f.write("\nNOTE: Engine run skipped intentionally.\n")

print(f"\n✓ Tauktae v1.3 script updated (run skipped).")

# ------------------------------------------------------------
# 7. Choropleths (only if run succeeded)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 8. Update district master (only if run succeeded)
# ------------------------------------------------------------
...
if merged_all is not None:
    ...
    print(f"\n✓ Updated district master with {len(out)} rows for {sid}")
else:
    print("\n⚠ Tauktae not added to district master (run skipped).")
