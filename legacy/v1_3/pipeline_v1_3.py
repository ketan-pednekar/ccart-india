...
# Import name normalizer + hazard floor constant from v1.2
from .engine_v1_2 import _normalize_name, HAZARD_FLOOR_MS
...
def run_cyclone_pipeline(
    ...
    """
    CCART v1.3 — State-agnostic, multi-state cyclone pipeline.

    Same scientific logic as v1.2, but now exports:
        - WindSpeed_Max_mps
        - dist_to_coast_km
        - is_inland
        - hazard_mask (explicit)
    """
    ...
    # ------------------------------------------------------------
    # 2. Inland clipping (distance to coast)
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # 5B. Add distance + inland flags + hazard_mask
    # ------------------------------------------------------------
    ...
    # ------------------------------------------------------------
    # 13. Merge calibrated + HWE + geometry + names
    # ------------------------------------------------------------
    print("districts:", districts.shape)
    print("district_loss_cal:", district_loss_cal.shape)
    print("hwe_weights:", hwe_weights.shape)
    ...
    # Build a small lookup table for names
    name_lookup = (
    ...
    # Map names without a heavy merge
    merged["district"] = merged["District"].map(name_lookup["district"])
    merged["state"] = merged["District"].map(name_lookup["state"])
