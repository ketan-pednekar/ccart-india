...
def ccart_choropleth(
    ...
    Supports PNG saving via save_path.
    """
    ...
    # ------------------------------------------------------------
    # 3.1 FIX GEOMETRY AFTER MERGE (critical)
    # ------------------------------------------------------------
    geom_cols = [c for c in gdf.columns if c.startswith("geometry")]

    if len(geom_cols) > 1:
        # Prefer the column literally named "geometry"
        if "geometry" in geom_cols:
            ...
            # Otherwise pick the first geometry-like column
            primary = geom_cols[0]
            gdf = gdf.set_geometry(primary)
            for c in geom_cols:
                if c != primary:
                    gdf = gdf.drop(columns=[c], errors="ignore")
    ...
    if save_path:
        ...
        plt.close()   # important for batch runs
    else:
        plt.show()
