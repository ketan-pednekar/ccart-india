"""
CCART v1.2 — State‑Agnostic Cyclone Pipeline
--------------------------------------------

National‑scale CCART engine:

- Runs cyclone impact for ALL India (no state filtering)
- Applies hazard floor, inland clipping, calibration, and HWE at national scale
- Automatically slices outputs by state after computation
- Compatible with historical, perturbed, and synthetic cyclones
"""
...
def run_cyclone_pipeline(
    ...
    """
    State‑agnostic CCART pipeline (v1.2).

    Runs the cyclone impact engine for ALL India, then slices
    outputs by state. This is the recommended entry point for
    historical, perturbed, and synthetic cyclone runs.
